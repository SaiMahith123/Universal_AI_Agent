...
with st.sidebar:
    ...
    model_choice = st.selectbox("Model:", ["llama-3.3-70b-versatile", "meta-llama/llama-4-scout-17b-16e-instruct", "deepseek-r1-distill-llama-70b"])
    st.divider()
...
prompt_data = st.chat_input("Ask or Upload...", accept_file=True, file_type=["pdf", "jpg", "png", "jpeg"], accept_audio=True)
...
if prompt_data:
    ...
    if st.session_state.vector_store:
        embed_model = get_embed_model()
        query_vector = embed_model.encode([user_text])
        D, I = st.session_state.vector_store["index"].search(np.array(query_vector).astype('float32'), k=3)
    ...
    if st.session_state.file_context["image_b64"]:
        ...
        messages = [{"role": "user", "content": [{"type": "text", "text": user_text}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{st.session_state.file_context['image_b64']}"}}]}]
    else:
        full_prompt = f"System: Use this context to answer.\nContext: {context}\n\nUser: {user_text}"
        messages = [{"role": "user", "content": full_prompt}]

    with st.chat_message("assistant"):
        try:
            stream = client.chat.completions.create(model=active_model, messages=messages, stream=True)
            def parse_stream(s):
                for chunk in s:
